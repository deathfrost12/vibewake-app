...
export default function AlarmsScreen() {
  const { alarms, isLoading, loadAlarms, toggleAlarm, deleteAlarm } = useAlarmStore();
  ...
  const renderDeleteAction = useCallback((alarmId: string) => {
    return (
      <TouchableOpacity
        style={{
  ...
  const renderAlarmItem = useCallback(({ item: alarm }: { item: Alarm }) => {
    return (
      <Swipeable
        key={alarm.id}
    ...
    <ThemedView style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1 }}>
        <FlatList
          ...
          ListEmptyComponent={() => (
            <ThemedCard style={{ padding: 24, alignItems: 'center', borderRadius: 12, marginHorizontal: 24 }}>
