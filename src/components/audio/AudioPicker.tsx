...
import {
  SoundLibrary,
  PredefinedSound,
} from '../../services/audio/SoundLibrary';
import {
  FileUploadService,
  UploadedAudioFile,
} from '../../services/audio/FileUploadService';
import {
  spotifyAuth,
  SpotifyTrack,
  SpotifyPlaylist,
...
import {
  getSelectedSpotifyTrack,
  clearSelectedSpotifyTrack,
} from '../../app/alarms/spotify-selector';
...
export function AudioPicker({
  ...
  const handleFileUpload = async () => {
    try {
      ...
      if (uploadedFile) {
        ...
        Alert.alert(
          'Success',
          `Audio file "${uploadedFile.originalName}" uploaded successfully!`
        );
      }
    } catch (error: any) {
  ...
  const renderTabButton = (
    ...
    <TouchableOpacity
      style={{
        ...
        backgroundColor:
          activeTab === tab ? (isDark ? '#1A2626' : '#F1F5F9') : 'transparent',
        borderRadius: 8,
        marginHorizontal: 4,
      }}
      ...
      <View style={{ alignItems: 'center' }}>
        ...
        <Text
          style={{
            ...
            marginTop: 4,
            color:
              activeTab === tab ? '#5CFFF0' : isDark ? '#A8B4B6' : '#64748B',
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      <View style={{ gap: 12 }}>
          ...
          >
            {isUploading ? (
              ...
              <View style={{ alignItems: 'center' }}>
                ...
                <ThemedText
                  style={{ marginTop: 8, fontSize: 14, fontWeight: '600' }}
                >
                  Upload Audio File
                </ThemedText>
                <ThemedText
        ...
        {uploadedFiles.length === 0 && !isUploading && (
          <ThemedCard
            style={{ padding: 24, alignItems: 'center', borderRadius: 12 }}
          >
            <Ionicons
              name="folder-open"
              size={32}
  ...
  const handleSpotifyLogin = async () => {
    try {
      ...
      Alert.alert(
        ...
        error.message || 'Unable to connect to Spotify'
      );
    } finally {
  ...
  const convertSpotifyToAudioTrack = (
    spotifyTrack: SpotifyTrack
  ): AudioTrack => ({
    id: `spotify-${spotifyTrack.id}`,
    name: `${spotifyTrack.name} - ${spotifyTrack.artists.map(a => a.name).join(', ')}`,
    uri: spotifyTrack.preview_url || '', // Use preview_url or empty string if not available
    type: 'spotify',
        ...
        <View style={{ gap: 16 }}>
          ...
          >
            ...
            <TouchableOpacity onPress={handleSpotifyLogout}>
              <ThemedText style={{ fontSize: 14, color: '#EF4444' }}>
                Logout
              </ThemedText>
            </TouchableOpacity>
          </View>
            ...
            <View>
              ...
              {spotifyTracks.map(track => {
                ...
                return (
                  ...
                  >
                    <TouchableOpacity
                      onPress={() => handleSelectAudio(audioTrack)}
                    >
                      <View
                        style={{
                          flexDirection: 'row',
